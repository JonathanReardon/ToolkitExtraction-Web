...
# The default extractor is set from the CLI entry‑point (`set_default_extractor`)
_default_extractor: "JSONDataExtractor | None" = None  # populated elsewhere
...
def int_eval(
    int_eval_output: list[dict] | pd.DataFrame,
    *,
    extractor: JSONDataExtractor | None = None,
) -> pd.DataFrame:
    extractor = extractor or _default_extractor
    if extractor is None:
        raise RuntimeError("No JSONDataExtractor supplied")

    int_eval_df = extractor.process_data(int_eval_output, "out_eval_raw")

    int_eval_df["eef_eval_raw"] = (
        int_eval_df["out_eval_raw"]
        .map(set(["Is this an EEF evaluation?"]).issubset)
        .astype(int)
        .replace({0: "No", 1: "Yes"})
    )

    ht_df   = extractor.process_ht(int_eval_output, "out_eval_ht")
    info_df = extractor.process_info(int_eval_output, "out_eval_info")

    out = pd.concat([int_eval_df, ht_df, info_df], axis=1, sort=False)[
        ["out_eval_raw", "out_eval_ht", "out_eval_info", "eef_eval_raw"]
    ]

    extractor.clean_up(out)
...
class JSONDataExtractor:
    def __init__(self, json_path: str | Path):
        ...
        self.data_file  = self.json_path.name      # if other code still reads it
    ...
    def verbose_display(self, df, n: int = 5) -> None:
            """
            Print the first *n* rows of *df* to stdout.
            In the Flask app nothing calls this, so it’s harmless.
            """
            try:
                ...
                # never blow up the caller – it’s only for debugging
                pass
    ...
    def getOutcomeData(
        ...
        out_label: str,
        out_container: list[list],
        var_names: list[str],
        *,
        extractor: JSONDataExtractor | None = None,
    ) -> list[list]:
        """
        Populate *out_container* with strand-level outcome data.

        Parameters
        ----------
        dataframe     : DataFrame that already contains columns like
                        ``out_type_1``, ``out_type_2`` …
        out_label     : Outcome-type label to look for (e.g. "Primary outcome")
        out_container : List of lists that will be *extended* in-place; each inner
                        list corresponds to a variable in *var_names*.
        var_names     : Column prefixes that match entries in *dataframe*.
        extractor     : JSONDataExtractor.  Omit it in the CLI (after calling
                        `set_default_extractor()`); pass it explicitly from Flask
                        or tests.
        """
        # ------------------------------------------------------------------
        # choose extractor
        ...
        if extractor is None:
            raise RuntimeError(
                "No JSONDataExtractor supplied – pass one explicitly or call "
                "set_default_extractor(path) first."
            )

        # ------------------------------------------------------------------
        # pull metadata & outcome–type frames
        # ------------------------------------------------------------------
        eppiid_df       = extractor.retrieve_metadata("ItemId",      "id")
        author_df       = extractor.retrieve_metadata("ShortTitle",  "pub_author")
        outcometype_df  = extractor.get_outcome_data_lvl2(
            outcome_type_codes, "out_type_"
        )
        outcome_df      = extractor.get_outcome_data_lvl1("OutcomeText", "out_label_")
        outcome_title_df = extractor.get_outcome_data_lvl1("Title", "out_tit_")

        df = pd.concat([outcometype_df, outcome_df, outcome_title_df], axis=1)[list(interleave([outcometype_df, outcome_df, outcome_title_df]))]

        all_vars = pd.concat([eppiid_df, author_df, df], axis=1, sort=False)

        # how many outcome columns exist?  → look at the last suffix
        outcome_num = int(all_vars.columns[-1].split("_")[-1])

        # ------------------------------------------------------------------
        # loop over rows and fill *out_container*
        # ------------------------------------------------------------------
        for idx in range(len(dataframe)):
            found = False
            for n in range(1, outcome_num + 1):

                value = dataframe[f"out_type_{n}"][idx]          # list or str/NaN
                candidates = value if isinstance(value, list) else [str(value)]

                if out_label in candidates:                      # ✅ now works
                    found = True
                    for j, holder in enumerate(out_container):
                        holder.append(dataframe[f"{var_names[j]}{n}"][idx])
                    break
    ...
    def save_dataframe(
            ...
            *,
            ...
            standard_info: bool = False,    # kept for API parity
            custom_info: bool = False,
        ):
            from pathlib import Path

            json_path = Path(self.json_path)         # attribute set in __init__
            ...
            outfile = output_dir / f"{json_path.stem}{suffix}"
            df.to_csv(outfile, index=False)

            # hooks preserved for future use
            if standard_info:
                pass
            if custom_info:
                pass
...
def save_dataframe(
    ...
    *,
    output_dir: str | Path | None = None,
    standard_info: bool = False,
    custom_info: bool = False,
) -> Path:
    """
    Write ``df`` to a CSV and return the full path.

    Parameters
    ----------
    df            : pandas.DataFrame
    df_name       : short suffix, e.g. "_tile3"
    json_path     : original .json file used to create the dataframe
    output_dir    : folder to write into (defaults to CWD)
    standard_info / custom_info : reserved flags, kept for API parity
    """
    ...
    # Build output filename:  study_123 + _tile3  → study_123_tile3.csv
    outfile_name = f"{json_path.stem}{df_name}.csv"
    outfile = output_dir / outfile_name

    df.to_csv(outfile, index=False)

    # (standard_info / custom_info hooks go here if you ever need them)
    return outfile
...
class DataFrameCompilation:
    def __init__(self, extractor: JSONDataExtractor):
        self.extractor = extractor
        self.data_extraction = extractor      # ← keep legacy name


    def make_dataframe_1(self, save_file=True, clean_cols=True, verbose=False):
        """
    ...
    def make_dataframe_2(self, save_file=True, clean_cols=True, verbose=False):
        """
        Extracts data from the sources defined in the current instance of the 
        DataExtraction class, and returns a pandas DataFrame with the relevant 
        variables for further cleaning and analysis.
        ...
        int_evaluation = int_eval(
            int_eval_output=int_eval_output,      # whatever variable already exists
    ...
    def make_dataframe_3(self, save_file=True, clean_cols=True, verbose=False):
        """
        Extracts data from the sources defined in the current instance of the 
        DataExtraction class, and returns a pandas DataFrame with the relevant 
        variables for further cleaning and analysis.
    ...
    def make_dataframe_4(self, save_file=True, clean_cols=True, verbose=False):
        """
        Extracts data from the sources defined in the current instance of the 
        DataExtraction class, and returns a pandas DataFrame with the relevant 
        variables for further cleaning and analysis.
    ...
    def make_dataframe_5(self, save_file=True, clean_cols=True, verbose=False):
        """
        ...
        """
        from eefdata.src.funcs import set_default_extractor
        set_default_extractor(self.extractor.json_path)   # works, extractor owns the path
        ...
        sample_check = sample_main_check(
            sample_output,                   # the same object used two lines above
            extractor=self.extractor,        # give the helper the JSONDataExtractor
        )
        ...
        # ↓ add the extractor on every call
        group1N_df   = group_desc_stats("Data1", "out_g1_n_",   extractor=self.extractor)
        group2N_df   = group_desc_stats("Data2", "out_g2_n_",   extractor=self.extractor)
        group1mean_df= group_desc_stats("Data3", "out_g1_mean_",extractor=self.extractor)
        group2mean_df= group_desc_stats("Data4", "out_g2_mean_",extractor=self.extractor)
        group1sd_df  = group_desc_stats("Data5", "out_g1_sd_",  extractor=self.extractor)
        group2sd_df  = group_desc_stats("Data6", "out_g2_sd_",  extractor=self.extractor)
    ...
    def make_dataframe_6(
            self,
            strand: str = "all",          # ← NEW required argument
            save_file: bool = True,
            clean_cols: bool = True,     # accepted but ignored
            output_dir: str | Path | None = None,
            verbose: bool = False,
        ):
            # ----------------------------------------------------------------
            # 1. wire helpers so group_desc_stats / getOutcomeData can see the
            #    extractor even when you call from Flask
            # ----------------------------------------------------------------
            set_default_extractor(self.data_extraction.json_path)

            # 2. pick the strand-specific block(s)
            ss = StrandSpecificFrames(self.data_extraction)

            match strand:
                case "arts_participation":
                    ss_df = ss.arts_participation_ss()
                case "behaviour_int":
                    ss_df = ss.behaviour_int_ss()
                case "collab_learning":
                    ss_df = ss.collab_learning_ss()
                case "early_literacy_approaches":
                    ss_df = ss.ey_early_lit_approaches_ss()
                case "early_numeracy_approaches":
                    ss_df = ss.ey_early_num_approaches_ss()
                case "extended_school_time":
                    ss_df = ss.ext_school_time_ss()
                case "earlier_starting_age":
                    ss_df = ss.ey_earlier_start_age_ss()
                case "play_based_learning":
                    ss_df = ss.ey_play_based_learning_ss()
                case "feedback":
                    ss_df = ss.feedback_ss()
                case "homework":
                    ss_df = ss.homework_ss()
                case "individual_instruction":
                    ss_df = ss.indiv_instr_ss()
                case "all":
                    ss_df = pd.concat(
                        [
                            ss.arts_participation_ss(),
                            ss.behaviour_int_ss(),
                            ss.collab_learning_ss(),
                        ],
                        axis=1,
                        sort=False,
                    )
    ...
    def make_dataframe_7(
        self,
        *,  # force keyword-only args
        save_file: bool = True,
        clean_cols: bool = True,  # ignored
        output_dir: str | Path | None = None,
        verbose: bool = False,
        ) -> tuple[pd.DataFrame, str]:
        """
        Generate the *Outcome Data* dataframe (tile 7) for Flask.

        Returns
        -------
        df : pandas.DataFrame
            The outcome-level DataFrame.
        csv_path : str
            Path to the saved CSV file.
        """
        # 1) Retrieve metadata and outcome data
        # Note: use self.data_extraction not a separate extractor variable
        eppiid_df = self.data_extraction.retrieve_metadata('ItemId', 'id')
        author_df = self.data_extraction.retrieve_metadata('ShortTitle', 'pub_author')
        # assume outcome_type_codes imported in module
        outcometype_df = self.data_extraction.get_outcome_data_lvl2(outcome_type_codes, 'out_type_')
        outcome_df = self.data_extraction.get_outcome_data_lvl1('OutcomeText', 'out_label_')
        outcome_title_df = self.data_extraction.get_outcome_data_lvl1('Title', 'out_tit_')

        # 2) Interleave columns if desired, otherwise concat
        # Here we simply concat side-by-side
        df_body = pd.concat([outcometype_df, outcome_df, outcome_title_df], axis=1, sort=False)

        # 3) Combine with metadata
        df = pd.concat([eppiid_df, author_df, df_body], axis=1, sort=False)

        # 4) Optional console preview
        if verbose:
            self.data_extraction.verbose_display(df)

        # 5) Persist CSV into output_dir
        out_path: str | None = None
        if save_file:
            out_path = self.data_extraction.save_dataframe(
                df,
                '_Outcomes.csv',
                standard_info=True,
    ...
    # ----------------------------------------------------------------------
    #  Tile 8  – Study security  /  Risk-of-Bias summary
    # ----------------------------------------------------------------------
    def make_dataframe_8(
        self,
        *,                       # ← all args keyword-only (mirrors other tiles)
        save_file: bool = True,
        clean_cols: bool = True,      # accepted for compatibility – ignored
        output_dir: str | Path | None = None,
        verbose: bool = False,
    ) -> tuple[pd.DataFrame, str]:
        """
        Generate the *Risk-of-Bias* dataframe (tile 8).

        Uses the RiskofBias helper class to produce the full risk_of_bias_df,
        including all post-processed metrics columns, and fits the api.create_csv() contract.
        """
        # 1) Initialize RiskofBias and build complete risk_of_bias_df
        rob = RiskofBias(self.data_extraction)
        rob.initialize_vars()
        rob.compile()  # runs through all rob_* methods and post-processing

        # 2) Extract the full risk_of_bias DataFrame
        df = rob.risk_of_bias_df.copy()

        # 3) Optional console preview
        if verbose:
            self.data_extraction.verbose_display(df)

        # 4) Persist CSV into output_dir
        csv_path: str | None = None
        if save_file:
            csv_path = self.data_extraction.save_dataframe(
                df,
                "_DataFrame8_StudySecurity.csv",
                standard_info=True,
                output_dir=output_dir,
            )
        return df, csv_path
    ...
    def make_dataframe_9(
        self,
        *,  # force all args to be keyword-only
        save_file: bool = True,
        clean_cols: bool = False,     # accepted for compatibility – ignored
        output_dir: str | Path | None = None,
        verbose: bool = False,
    ) -> tuple[pd.DataFrame, str]:

        # 1) Initialize and prepare RiskofBias
        rob = RiskofBias(self.data_extraction)
        rob.initialize_vars()
        rob.compile()

        # 2) Let padlocks generate its CSV and return its path
        csv_path = rob.padlocks()
        # 3) Load it back into a DataFrame
        ...
        # 5) Persist into the provided output_dir (reuse naming)
        out_path: str | None = None
        if save_file:
            out_path = self.data_extraction.save_dataframe(
                df,
    ...
    def make_dataframe_10(
            ...
            *,
            ...
            """
            Generate the *References* dataframe (tile 10) for Flask.
            ...
            csv_path : str
                Path to the saved CSV file.
            """
            # 1) Retrieve all relevant metadata fields
            eppiid_df = self.data_extraction.retrieve_metadata("ItemId", "id")
            ...
            parent_title_df = self.data_extraction.retrieve_metadata("ParentTitle", "parent_title")
            parent_authors_df = self.data_extraction.retrieve_metadata("ParentAuthors", "parent_authors")
            type_name_df = self.data_extraction.retrieve_metadata("TypeName", "type_name")
            abstract_df = self.data_extraction.retrieve_metadata("Abstract", "abstract")
...
class RiskofBias:
    def __init__(self, data_extractor):
        # canonical name used by all new code
        self.data_extraction = data_extractor

        # ***legacy alias*** — some old helpers still expect ".extractor"
...
def group_desc_stats(
    ...
    *,
    ...
    """
    Return a tidy DataFrame of strand-level descriptors.

    Parameters
    ----------
    attribute_text : field to pull from JSON, e.g. "Age"
    column_prefix  : prefix for the generated columns, e.g. "age_"
    extractor      : optional JSONDataExtractor.  If omitted the function
                     will use the module-level `_default_extractor` that the
                     CLI sets via `set_default_extractor()`.
    """
    # ----------------------------------
    # choose extractor
    # ----------------------------------
    ...
    if extractor is None:
        raise RuntimeError(
            "No JSONDataExtractor supplied – pass one explicitly or call "
            "set_default_extractor(path) before importing this helper."
        )

    # ----------------------------------
    # build the frame
    # ----------------------------------
    group_data_df = pd.DataFrame(extractor.get_outcome_lvl1(attribute_text))
    group_data_df.columns = [
        f"{column_prefix}{i+1}" for i in range(group_data_df.shape[1])
    ]
    group_data_df.replace(r"^\s*$", "NA", regex=True, inplace=True)

    # 0-mask handling
    outtype_df = pd.DataFrame(extractor.get_outcome_lvl1("OutcomeTypeId"))
    mask = outtype_df.iloc[:, 0] == 0
    group_data_df.loc[mask, :] = "NA"
...
def sample_main_check(
    sample_output: list[dict] | pd.DataFrame,
    *,
    extractor: JSONDataExtractor | None = None,
) -> pd.DataFrame:
    """
    Return a 1-row DataFrame telling whether each study’s sample is the
    ‘main’ one.

    Parameters
    ----------
    sample_output : raw JSON chunk or pre-built DataFrame that
                    `get_data()` expects.
    extractor     : optional JSONDataExtractor.  Omit it in the CLI after
                    calling `set_default_extractor()`, pass it explicitly
                    from Flask/tests.
    """
    # ---------------------------------------------------------------
    # choose extractor
    ...
    # original logic
    # ---------------------------------------------------------------
    main_chk_df = pd.DataFrame(extractor.get_data(sample_output)).T
    main_chk_df.columns = ["main_check"]
    main_chk_df.fillna("NA", inplace=True)
    return main_chk_df
...
# -------------------------------------------------------------------
# Runtime-injected extractor (used only if caller doesn’t pass one)
# -------------------------------------------------------------------
_default_extractor: JSONDataExtractor | None = None

def set_default_extractor(path: str | Path) -> None:
    """CLI helper – set once, then helpers fall back to it."""
    global _default_extractor
    _default_extractor = JSONDataExtractor(path)
